...
def parse_input(input_data:str) -> list[tuple[int, int]]:
    ...
    red_tiles_with_correct_type = typing.cast(list[tuple[int, int]], red_tiles)
    return red_tiles_with_correct_type
...
def border_intersects_with_inside_of_rectangle(
    ...
    for i, _ in enumerate(red_tiles):
        ...
        if borderline_intersects_with_inside_of_rectangle(corners, red_tile1,
                                                          red_tile2):
            return True
    return False
...
def find_largest_green_rectangle_by_area(
    ...
    for tile_pair in ordered_by_area_red_tile_pairs:
        if is_green_rectangle(tile_pair, red_tiles):
            return tile_pair
    ## this should never run for a proper puzzle input
    return ((0, 0), (0, 0))
